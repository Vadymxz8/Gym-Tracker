...
public class WorkoutExerciseMapper {
    ...
    public ExerciseEntity toExerciseEntity(Exercise exercise) {
    ...
    public WorkoutExercise toWorkoutExercise(WorkoutExerciseInputDto dto) {
        ...
        return WorkoutExercise.builder().exerciseId(dto.getExerciseId()).sets(dto.getSets()).reps(dto.getReps()).weight(dto.getWeight()).note(dto.getNote()).workoutId(dto.getWorkoutId()).build();
    }

    public WorkoutExerciseInputDto toWorkoutExerciseInputDto(WorkoutExercise exercise) {
        if (exercise == null) return null;

        return WorkoutExerciseInputDto.builder().exerciseId(exercise.getExerciseId()).sets(exercise.getSets()).reps(exercise.getReps()).weight(exercise.getWeight()).note(exercise.getNote()).workoutId(exercise.getWorkoutId()).build();
    }

    public WorkoutExercise toWorkoutExercise(WorkoutExerciseEntity entity) {
        if (entity == null) return null;

        return WorkoutExercise.builder().id(entity.getId()).exerciseId(entity.getExercise().getId()).sets(entity.getSets()).reps(entity.getReps()).weight(entity.getWeight()).note(entity.getNote()).workoutId(entity.getWorkout() != null ? entity.getWorkout().getId() : null).build();
    }
